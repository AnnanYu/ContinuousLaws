#!/usr/bin/env python3
"""Train an S6 model to infer mu from 1D Van der Pol oscillator time series.
...
Example:
    python train_DS_mamba.py --data data/vdp_mu.npz --epochs 50 --batch_size 128 --d_model 128 --n_layers 4
"""
...
class S6Regressor(nn.Module):
    def __init__(self, d_input=1, d_output=1, d_model=128, n_layers=4, dropout=0.1, prenorm=False):
        super().__init__()
...
def main():
    p = argparse.ArgumentParser(description="Train S6 on dynamical systems data.")
