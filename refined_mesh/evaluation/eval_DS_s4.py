#!/usr/bin/env python3
"""Evaluate an S4D model to infer mu from 1D Van der Pol oscillator time series.
...
class S4Regressor(nn.Module):
    def __init__(self, d_input=1, d_output=1, d_model=128, n_layers=4, dropout=0.1, prenorm=False, s4_lr=1e-3, scale_dt=1.0):
        super().__init__()
        ...
        for _ in range(n_layers):
            self.s4_layers.append(S4D(d_model, dropout=dropout, transposed=True, lr=s4_lr, scale_dt=scale_dt))
...
def main():
    p = argparse.ArgumentParser(description="Evaluate S4D on dynamical systems data.")
